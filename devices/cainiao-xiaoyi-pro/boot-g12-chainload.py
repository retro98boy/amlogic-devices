...
def parse_cmdline(fpath):
    ...
    parser.add_argument('chainload_binary',  action='store',
                        help="binary to chainload")
...
if __name__ == '__main__':
    ...
    chainload_path = args.chainload_binary
    ...
    with open(chainload_path, "rb") as f:
        data = f.read()

    print("Waiting for vendor U-Boot")
    time.sleep(10)

    block_size = 1024 * 1024 # 字节
    download_ddr_address = 0x1000000

    dev = pyamlboot.AmlogicSoC()

    print(f"Downloading binary to {hex(download_ddr_address)}...")
    dev.writeLargeMemory(download_ddr_address, data, block_size, True)
    print(f"Goto {hex(download_ddr_address)}...")
    dev.bulkCmd(f"go {hex(download_ddr_address)}")
